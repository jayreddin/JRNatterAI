...
function togglePopup(name, open) {
  ...
  if(open){
    ...
    if(name==='code'){
      document.getElementById('code-gen-prompt').value='';
      document.getElementById('code-result').textContent='';
    }
  }
}
...
window.copyOCR = function(){
  navigator.clipboard.writeText(document.getElementById('ocrTextResult').textContent);
};

window.editOCR = function(){
  let cur = document.getElementById('ocrTextResult').textContent;
  document.getElementById('file-preview-box').innerHTML += `<textarea class='flat rounded-cool w-full mt-2' rows='3' id='ocrEdit'>${cur}</textarea><button onclick="saveOCR()" class="feature-btn mt-2"><i class="fa fa-save"></i></button>`;
};
